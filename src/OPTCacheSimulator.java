 ...
 *
 * Description:  Class that simulates a chache with OPT replacement policy.
 ***************************************************************************/
...
public class OPTCacheSimulator{
	...
	/**
	 * Method that determines whether a reference is a hit or miss in the cache. Automatically updates the symbol table and cache.
	 * 
	 * @param address The address being accessed
	 * @param stackDist Represents the priority of the address
	 * @return true is returned if there is a hit, false otherwise.
	 */
	public boolean access(int address, int stackDist){
		if(symbolTable.hasSymbol(address)) {
		...
		else if(!symbolTable.hasSymbol(address) && cache.size() != cacheCapacity){
			...
			symbolTable.removeSymbol(cache.remove());
			cache.insert(address, stackDist);
			symbolTable.putRecord(address, stackDist);
			cache.updatePriority(address, stackDist);
			return false;
